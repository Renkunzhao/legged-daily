...
- 招聘信号仅复核到 EPFL BioRob 的官方实验室方向与非官方转载 opening；未找到官方原始广告，暂列为 tentative。
...
- [BotBrain](https://github.com/botbotrobotics/BotBrain)
  ...
  Importance: medium
  Summary: 2026-01 创建、2026-05 活跃更新的模块化 ROS2 robot brain，面向 quadruped、biped、humanoid 与 wheeled robots，提供 Web UI、teleop、navigation、mapping、monitoring 和 3D-printable hardware；README 明确列出 Unitree Go2、Unitree G1、DirectDrive Tita 支持。

- [Crocoddyl](https://github.com/loco-3d/crocoddyl)
  Topic: optimal control / DDP / contact dynamics / toolkit
  Importance: medium
  Summary: 经典接触序列最优控制库，提供 DDP-like solvers、Pinocchio dynamics/derivatives、Python bindings 与多种 constraints/costs；本轮 GitHub API 显示 2026-05 仍有 push/update，适合作为 legged whole-body control / trajectory optimization 基础工具继续跟踪。

- [Upkie](https://github.com/upkie/upkie)
  Topic: wheeled biped / open hardware / simulation-to-hardware / Gymnasium
  Importance: low
  Summary: 开源 wheeled biped robot 平台，包含硬件构建材料、Python/C++ 控制、PyBullet/Gymnasium 环境以及仿真到实机切换路径；适合作为低成本 biped platform reference。
...
- Institution / Company / Lab / Team: EPFL Biorobotics Laboratory / Auke Ijspeert
  Type: PhD / Postdoc
  Source: non-official reposted job signal; official BioRob homepage verified, original opening not verified in this run
  Deadline: unknown
  Summary: 转载信息称 Fall 2026 有 human locomotion neuromechanics + humanoid robots 项目，方向覆盖 computational modeling、deep RL、bio-inspired humanoid locomotion control；因未找到官方原始广告，暂作为 tentative signal，不建议直接写入正式 jobs.md 前先二次验证。
